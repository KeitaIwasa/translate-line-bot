...
- [ ] フロー制御: 無料枠超過時は翻訳を中断し課金案内メッセージを返信、支払い後は「利用再開」メッセージを送信する分岐をメッセージ処理に組み込む。
- [ ] 設定/Secrets: Stripe API Key・Webhook Secret を Secrets Manager 連携に追加し、`runtime-secret`/環境変数に反映。プラン価格 ID を設定ファイルに保持。
- [ ] テスト: Stripe SDK をモックした単体テスト、Webhook イベントの疑似ペイロードを用いた統合テストを追加。既存テストとの整合を確認。
- [ ] ドキュメント: `README`/`AGENTS` に運用手順（無料枠、支払い後の挙動、価格 ID 差し替え手順、Webhook 構築手順）を追記。
